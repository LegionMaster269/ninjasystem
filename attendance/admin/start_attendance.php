...
if (isset($_POST['id_number']) && isset($_POST['lastname']) && isset($_POST['firstname']) && isset($_POST['department']) && isset($_POST['program']) && isset($_POST['event']) && isset($_POST['time']) && isset($_POST['date_inputed'])) {
...
$date_inputed = SanitizeString($_POST['date_inputed']);
...
window.location = 'start_attendance.php';
       });
       });
      </script>"; 
}
  else {
    QueryMysql("INSERT INTO reports(id_number,lastname,firstname,department,program,event,date_inputed ,time) VALUES('$id_number','$lastname','$firstname','$department','$program','$events','$date_inputed','$time')");
...
<html>
 <head>
  ...
  <title>Start Attendance</title>
   <Style>
     .input-box{margin-top:70px !important;}
 a{
...
function goBack() {
 ...
 <body>
 <a onclick="goBack()" ><i class="fas fa-home" style ="font-size:20px;"> &nbsp; </i>Home</a> 
  <div class="container" style="width:900px;">
    ...
    <form action="" method="post" class = "input-box">
      ...
      <select name="event" class="selection" style="color: black;">
          <?php
            $dates = queryMysql("SELECT date1 FROM events");
            $result = QueryMysql("SELECT * FROM events");
           
              while ($row = $result->fetch_assoc()) {
                $record = $dates->fetch_array();
                $date1 = $record['date1'];
                if($date1 == date('Y-m-d')) {   
             ?>
      ...
      <input type="text" name="id_number" id="id_number" placeholder="Search Employee Details" class="form-control" required/>
      ...
      <input type="hidden" name="date_inputed" value="<?php echo date('Y-m-d');?>" placeholder="Search Employee Details" class="form-control" />
      <input type="hidden" name="time" placeholder="Time" style="color: black;" value="<?php echo date("h:i:sa");?>">
