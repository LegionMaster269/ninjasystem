...
?>
 ...
 <?php
  ...
  $sql =  queryMysql("SELECT user FROM users WHERE user='$username'");
 ...
 queryMysql("INSERT INTO users(user, pass,type, course) VALUES('$username', '$password', 2,'$course')");        
				echo "<script>
      $(function() {
...
<div class=" col-3 col-t-2 nav-area">
    ...
    <div class=" menu">
	<button id="adduserbtn" onclick="document.getElementById('id01').style.display='block'" style="width:100%;" class="button"><i class="fas fa-user-plus"></i>  &nbsp; <label class= "button-label" id="adduserheader">Add User</label></button>
    <div id="id01" class="modal">
	<form class="modal-content animate" action="manage_user.php" method="post">
...
<section id="burn">
	<div class="container">
		<h2 id="header1">List of Users</h2>
	</div>
</section>
		...
		<div class="container">
			<div class="row">
						...
						<tbody>
							<?php
								$sql="SELECT * from users";
									...
									if(isset($_GET['del'])) 
									{
										$user = sanitizeString($_GET['del']);
										
										queryMysql("DELETE FROM users WHERE user='$user'");
										echo "<script>
										$(function() {
								...
								if (isset($_POST['update'])) {
									...
									$username = sanitizeString($_POST['user']);
									$password = sanitizeString($_POST['pass']);
									$result = queryMysql("SELECT user FROM users WHERE user='$user'");

									if($_POST['update']) {
										
					queryMysql("UPDATE users SET user='$username', pass='$password' WHERE user='$username'");
					if($result->num_rows > 1) {
					...
					} else
					 {
					  ...
					  window.location = 'manage_user.php';
								...
								if (isset($_GET['edit'])) {
  								$username = sanitizeString($_GET['edit']);
  			$rec = queryMysql("SELECT * FROM users WHERE user='$username'");
  			...
  			$password = $record['pass'];
								...
								$result = queryMysql("SELECT * FROM users limit 1,1000000");
								...
								<td><a href="edit.php?edit=<?php echo $row['user'];?>" >UPDATE</a></td>
								<td><a type="button" href="manage_user.php?del=<?php echo $row['user'];?>" >DELETE</a></td>
  								</form>
								</div>
